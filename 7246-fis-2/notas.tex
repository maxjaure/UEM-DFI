...
Suponhamos que uma onda transversal senoidal se propague através de
uma corda tensionada de densidade linear de massa $\rho(x)$ e
consideremos uma porção da corda como ilustrado na
...
$\vec T(x)$, no seu extremo esquerdo, e $\vec T(x+\Delta x)$, no
...
e
\begin{multline*}
  T(x+\Delta x)\sen\theta(x+\Delta x)\\
  -T(x)\sen\theta(x)=\rho(x)\sqrt{\Delta x^2+\Delta
...
\begin{equation*}
  \begin{split}
    \frac{\partial}{\partial x}[T\cos\theta]&=0\\
    \frac{\partial}{\partial
      x}[T\sen\theta]&=\rho(x)\sqrt{1+\dpar{\frac{\partial y}{\partial
...
\begin{figure}[ht]
  ...
  \caption{Porção de uma corda tensionada, pela qual passa uma onda
    senoidal.}
