...
\setmathfont{TeX Gyre Termes Math}[range={up,it}]
...
atua uma força $\vec F$. O \emph{torque} da força $\vec F$ é
definido por $\vec\tau=\vec r\times\vec F$, onde $\times$ denota o
\emph{produto vetorial} de dois vetores, o qual é definido por
\begin{equation*}
  \begin{split}
...
\begin{ex}
  ...
  qual atua uma força $\vec F$. Definindo o \emph{momento angular}
...
Um \emph{corpo rígido} é um corpo não pontual que não se deforma. O
...
\emph{momento linear do corpo rígido} é dado por
\begin{equation}
  \label{eq:4}
  \vec p=\int \vec v\,dm
\end{equation}
e o \emph{momento angular do corpo rígido} por
\begin{equation}
  \label{eq:5}
...
\begin{ex}
  ...
  é a posição do \emph{centro de massa} do corpo.
...
\end{equation}
é o chamado \emph{momento de inércia} do corpo. Cabe ressaltar que
...
Um corpo rígido está em \emph{equilíbrio estático} quando não
...
Define-se o \emph{centro de gravidade} de um corpo rígido de massa
...
\emph{força gravitacional}. Mais precisamente, ele encontrou a
...
onde $G$ é uma constante chamada de \emph{constante gravitacional}.
...
energia potencial. De fato, se definimos a \emph{energia potencial
...
chamado de \emph{gradiente} de $V_g$ e é usualmente denotado em
...
é chamada de \emph{peso}. Nesse caso, o peso da partícula segundo a
...
\begin{ex}
  ...
  chamada de \emph{velocidade de escape}.
...
\begin{ex}
  ...
  $$v=\sqrt{\frac{GM_T}{r}}\,.$$
  Definindo o \emph{período} $T$ como o tempo que a partícula demora
...
\begin{ex}
  ...
  (\emph{perihélio}), a distância entre o cometa e o Sol é de
  $8{,}75\times 10^7\,\mathrm{km}$. No instante em que o cometa está
  mais afastado do Sol (\emph{afélio}), a distância entre o cometa e
