...
Consideremos uma partícula de massa $m$ a uma distância $r$ do centro
da Terra. Se $\vec F_g$ é a força gravitacional sobre a partícula,
devido à Terra, então o vetor $\vec F_g$ é proporcional ao vetor
$-\vec r$. Dessa maneira, o torque de $\vec F_g$ vai ser nulo, pois é
o produto vetorial de vetores paralelos. Se $\vec F_g$ é a única força
que atua sobre a partícula, segue então da Eq.~(\ref{eq:3}) que o
momento angular da partícula $\vec L=\vec r\times (m\dot{\vec r})$ se
conserva, ou seja, $\vec L$ é um vetor constante (módulo e direção
constantes). Em particular, isso implica que o vetor $\vec L$ vai ser
sempre perpendicular aos vetores $\vec r$ e $\dot{\vec r}$ e, por
conseguinte, o movimento da partícula é realizado em um plano que é
...
\begin{figure}[ht]
  ...
  \caption{O movimento da partícula é realizado no plano $xy$.}
  \label{fig:leikepler2r}
\end{figure}

Da figura~\ref{fig:leikepler2r} vemos que
$\vec r=r\cos\theta\hat i+r\sen\theta\hat j$ e, por conseguinte,
$\dot{\vec r}=-r\dot\theta\sen\theta\hat i+r\dot\theta\cos\theta\hat
j$. Logo,
$$\vec L=m\vec r\times\dot{\vec r}=m\left|
  \begin{array}{ccc}
    \hat i&\hat j&\hat k\\
    r\cos\theta& r\sen\theta& 0\\
    -r\dot\theta\sen\theta& r\dot\theta\cos\theta&0
  \end{array}
\right|\,.$$ Portanto,
...
varrida pelo vetor posição da partícula entre dois instantes de tempo
